...
class DirectXExporter:
    def __init__(self, Config, context):
        ...
        if self.Config.CoordinateSystem == 'LEFT_HANDED':
            self.SystemMatrix = self.SystemMatrix @ Matrix.Scale(-1, 4, (0, 0, 1))

        if self.Config.UpAxis == 'Y':
           self.SystemMatrix = self.SystemMatrix @  Matrix.Rotation(radians(-90), 4, 'X')
			
        #self.SystemMatrix = self.SystemMatrix @ s_Scale @ s_Rotate
    ...
    def Write(self):
        ...
        if self.Config.ExportMeshes:
            ...
            if self.Config.ApplyModifiers:
                ...
                Mesh = self.BlenderObject.to_mesh(preserve_all_data_layers=False, depsgraph=None)
        ...
        def WriteMaterial(Exporter, Material):
            def GetMaterialTextureFileName(Material):
                if Material:
                    ...
                    ImageTextures = [Material.texture_paint_slots[TextureSlot].texture
                        for TextureSlot in Material.texture_paint_slots.keys()
                        if Material.texture_paint_slots[TextureSlot].texture.type ==
                        'IMAGE']
    ...
    def _GenerateBoneKeys(self):
        ...
        for Frame in range(Scene.frame_start, Scene.frame_end + 1):
            ...
            for Bone, BoneAnimation in \
                ...
                PoseMatrix = PoseMatrix @ Bone.matrix
